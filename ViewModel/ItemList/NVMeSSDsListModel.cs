...
namespace GenosStore.ViewModel.ItemList {
	public class NVMeSSDsListModel: ComputerComponentListViewModel<NVMeSSD> {
		...
		protected override string _itemPageURL {
			get {
				return "View/ItemPage/NVMeSSDPage.xaml";
			}
		}

		protected override AbstractViewModel _itemPageViewModel(int id) {
			return new NVMeSSDPageModel(_services, _user, id);
		}
		...
		protected override void ApplyFilters(object parameter) {
			...
			var filters = new List<Func<NVMeSSD, bool>>();
			...
			Items = Utilities.ConvertAndCheckDiscounts(
				...
				_services.Entity.Items.ComputerComponents.NVMeSSDs
			);
		...
		protected override List<NVMeSSD> _getItems() {
			return _services.Entity.Items.ComputerComponents.NVMeSSDs.List();
		}
		...
		public NVMeSSDsListModel(IServices services, User user): base(services, user) {
			...
			Items = GetItemsAndCheckDiscounts(
				_services.Entity.Items.ComputerComponents.NVMeSSDs.List()
			);
